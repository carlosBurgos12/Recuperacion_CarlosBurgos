...
export default function Productos({ navigation }) {
  ...
  const getProductos = async (idCategoriaSelect = 1) => {
    try {
      ...
      const response = await fetch(`${ip}/recuperacion/api/services/public/producto.php?action=readProductosCategoria`, {
        method: 'POST',
        body: formData
      });
  ...
  const getCategorias = async () => {
    try {
      ...
      //utilizar la direccion IP del servidor y no localhost
      const response = await fetch(`${ip}/recuperacion/api/services/public/categoria.php?action=readAll`, {
        method: 'GET',
      });
