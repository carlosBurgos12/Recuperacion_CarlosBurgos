...
const Carrito = ({ navigation }) => {
  ...
  const getDetalleCarrito = async () => {
    try {
      const response = await fetch(`${ip}/recuperacion/api/services/public/pedido.php?action=readDetail`, {
        method: 'GET',
      });
  ...
  const finalizarPedido = async () => {
    try {
      const response = await fetch(`${ip}/recuperacion/api/services/public/pedido.php?action=finishOrder`, {
        method: 'GET',
      });
      const data = await response.json();
      if (data.status) {
